...
## Next
- Blewu (renamed from Needle & Sign, folder `blewu/`) v8 is live (artifact v9, 24 Sep 2026): swipe to steer on touch, swipeable car/road rows with drawn thumbnails. Host decided: play.kaditay.com (games hub), Blewu first. Earlier: v7: rain mode (braking x2), dramatic crashes with head-on explainer, quieter wind, 7 roads incl. Tema-Aflao, Ashaiman-Akosombo, Akosombo-Ho (Adomi Bridge). Open decision: final name and host (recommended: own subdomain on kaditay.com via Cloudflare Pages, like storeroom/marketing).
